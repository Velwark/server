	...
	public function run(): SetupResult {
		...
		if ($configValue === null) {
			return SetupResult::warning(
				$this->l10n->t('Server has no maintenance window start time configured. This means resource intensive daily background jobs will also be executed during your main usage time. We recommended to set it to a time of low usage, so users are less impacted by the load caused from these heavy tasks.'),
